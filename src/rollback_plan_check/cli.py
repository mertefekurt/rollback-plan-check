...
from rollback_plan_check import __version__
from rollback_plan_check.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from rollback_plan_check.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='rollback-plan-check',
        description=(
    ...
    parser.add_argument("input", nargs="?", type=Path, help='rollback plan')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"rollback-plan-check: error: {exc}", file=sys.stderr)
        return 1
